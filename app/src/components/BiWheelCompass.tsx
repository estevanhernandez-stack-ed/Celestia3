...
const BiWheelCompass: React.FC<BiWheelCompassProps> = ({ 
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto">
      ...
      {/* Hover Info Tooltips (Radial & Responsive) */}
      <div className="absolute inset-0 pointer-events-none z-50">
        <AnimatePresence>
          {hoveredPlanet && (
            (() => {
              const nodes = hoveredPlanet.type === 'inner' ? innerNodes : outerNodes;
              const node = nodes.find(n => n.name === hoveredPlanet.name);
              if (!node) return null;

              const isInner = hoveredPlanet.type === 'inner';
              const dist = isInner ? radius - 40 : radius + 25;
              const tx = cx + Math.cos(node.angle) * dist;
              const ty = cy - Math.sin(node.angle) * dist;

              return (
                <motion.div
                  key={`${hoveredPlanet.type}-${hoveredPlanet.name}`}
                  ...
                  className={`px-3 py-2 rounded-xl border backdrop-blur-xl shadow-2xl min-w-[110px] text-center flex flex-col items-center gap-1 ${
                    isInner 
                      ? 'bg-emerald-950/90 border-emerald-500/50 text-emerald-100 shadow-emerald-500/20' 
                      : 'bg-blue-950/90 border-blue-500/50 text-blue-100 shadow-blue-500/20'
                  }`}
                >
                  <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${
                    isInner ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'
                  }`}>
                    {isInner ? innerLabel : outerLabel}
                  </span>
                  <div className="font-bold text-xs uppercase tracking-tight">{node.name}</div>
                  <div className="h-px w-8 bg-current opacity-20" />
                  <div className="text-[9px] font-medium">
                    {node.sign} {node.degree.toFixed(2)}°
                  </div>
                </motion.div>
              );
            })()
