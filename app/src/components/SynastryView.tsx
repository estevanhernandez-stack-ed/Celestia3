...
const SynastryView: React.FC<SynastryViewProps> = ({ userChart }) => {
  ...
  // Retroactive XP for existing saved charts
  useEffect(() => {
    if (preferences.savedCharts && preferences.savedCharts.length > 0) {
        const unrewardedCharts = preferences.savedCharts.filter(c => !c.xpAwarded);
        
        if (unrewardedCharts.length > 0) {
            console.log(`Found ${unrewardedCharts.length} unrewarded synastry charts. Awarding retroactive XP.`);
            
            // Calculate total XP
            const xpPerChart = ProgressionService.calculateGainedXP('synastry_match');
            const totalXP = unrewardedCharts.length * xpPerChart;
            
            // 1. Award XP
            const currentXP = preferences.xp || 0;
            const currentLevel = preferences.level || 1;
            
            // Manually calc progression since addXP handles single action usually, 
            // but we can just loop or simplify. Let's use internal logic to avoid spamming level ups visually if multiple
            // actually addXP handles overflow loops. use it once with total? 
            // ProgressionService doesn't support "amount" arg, only action.
            // Let's modify logic or just iterate. Iterating might be safer to reuse strict logic.
            // Actually, safe way:
            ...
            // 2. Mark matches as awarded
            const updatedCharts = preferences.savedCharts.map(c => ({
                ...c,
                xpAwarded: true
            }));
  ...
  }, [preferences.savedCharts, preferences.xp, preferences.level, updatePreferences]); // Checks on mount and sync
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      // Award XP for creating a connection (Match)
      // Milestone: First match triggers Level 3
      const isMilestone = preferences.level === 2;
      const progression = ProgressionService.addXP(preferences, 'synastry_match', isMilestone);
  ...
  const handleSaveConnection = () => {
    ...
    const chartToSave: SavedChart = {
        ...
        xpAwarded: true // Mark as awarded since we just got it for 'synastry_match'
    };
  ...
  const handleAnalyzeConnection = async () => {
    ...
    try {
        ...
        // Award XP for generating a report
        const progression = ProgressionService.addXP(preferences, 'synastry');
        updatePreferences({ xp: progression.xp, level: progression.level });
    } catch {
  ...
  if (!userChart) return (
     <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center space-y-8 animate-in fade-in duration-700">
         <div className="relative">
             <Heart className="text-pink-500/20 fill-pink-500/10 animate-pulse" size={120} />
             <User className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-pink-400/50" size={40} />
         </div>
         <div className="space-y-4 max-w-md">
             <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Connection Impossible</h2>
             <p className="text-pink-200/60 leading-relaxed font-medium">
                 We cannot bridge the gap between souls until your own celestial anchor is fixed. 
                 Calibrate your birth data to enable compatibility resonance.
             </p>
         </div>
         <button 
             onClick={() => {
             ...
             className="px-10 py-4 bg-pink-600 hover:bg-pink-500 text-white rounded-2xl font-black uppercase tracking-[0.2em] text-xs transition-all shadow-xl shadow-pink-500/20 active:scale-95 group flex items-center gap-3"
         >
             Anchor Your Soul
