  ...
  useEffect(() => {
    if (hoveredPlanet && meditationStartTime && !meditatedPlanets.has(hoveredPlanet)) {
      const timer = setTimeout(() => {
        // Milestone: First exploration triggers Level 2
        const isMilestone = preferences.level === 1;
        const progression = ProgressionService.addXP(preferences, 'meditation', isMilestone);
    ...
    <div className="relative w-full max-w-2xl mx-auto group aspect-square">
          ...
          return (
            <motion.g 
              ...
              onMouseEnter={() => {
                ...
                ResonanceService.startDrone(p.name); // Start audio drone
              }}
              onMouseLeave={() => {
                setHoveredPlanet(null);
                setMeditationStartTime(null);
                ResonanceService.stopDrone(); // Stop audio drone
              }}
      ...
      <div className="absolute inset-0 pointer-events-none z-40">
        {planetNodes.map((p) => {
          ...
          // Tooltip position: Radially outward from the planet
          const tooltipDist = getPlanetRadius(p.name) + 45;
          const tx = cx + Math.cos(p.angle) * tooltipDist;
          const ty = cy - Math.sin(p.angle) * tooltipDist;
            ...
            <AnimatePresence key={`label-${p.name}`}>
              {isHovered && (
                <motion.div
                  ...
                  className="px-3 py-2 bg-black/90 backdrop-blur-xl border border-indigo-500/50 rounded-xl shadow-[0_0_20px_rgba(99,102,241,0.3)] min-w-[120px] z-50 pointer-events-none"
                >
                  <div className="flex flex-col items-center text-center space-y-1">
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-indigo-400 font-black uppercase tracking-widest">{p.name}</span>
                        <span className="text-[9px] text-fuchsia-500 font-bold px-1.5 py-0.5 bg-fuchsia-500/10 rounded">H{p.house}</span>
                    </div>
                    ...
                    <span className="text-[8px] text-indigo-300/60 uppercase tracking-tighter">Geometric Position</span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <div className="absolute top-0 left-0 p-4 pointer-events-none z-50">
        {/* Detail box removed to anchor info to planets */}
      </div>
