...
type DashboardView = 'compass' | 'synastry' | 'tarot' | 'athanor' | 'rituals' | 'chronos' | 'numerology' | 'grimoire' | 'codex' | 'admin' | 'celebrities';
...
const DashboardShell: React.FC = () => {
  ...
  const { preferences, updatePreferences, isHydrated } = useSettings();
  ...
  // Audio Context Gesture Listener
  useEffect(() => {
    const handleGesture = () => {
        ResonanceService.resume();
  ...
  // Trigger Onboarding for new users (wait for auth/profile hydration)
  useEffect(() => {
    if (isHydrated && !loading && user && !preferences.hasCompletedOnboarding) {
        setIsOnboardingOpen(true);
    }
  }, [user, preferences.hasCompletedOnboarding, loading, isHydrated]);
  ...
  const handlePerformRitual = async (intent: string, paradigm: string) => {
    ...
    try {
        ...
        // Add XP
        // Milestone: First ritual triggers Level 7
        const isMilestone = preferences.level === 6;
        const progression = ProgressionService.addXP(preferences, 'ritual', isMilestone);
  ...
  const handleTarotDraw = async (cards: TarotCard[], spreadId: string, intent: string) => {
     ...
     // Add XP
     // Milestone: First draw triggers Level 5
     const isMilestone = preferences.level === 4;
     const progression = ProgressionService.addXP(preferences, 'tarot', isMilestone);
     updatePreferences({ xp: progression.xp, level: progression.level });
  };

  // Handler for Aura Cam
  const handleSaveAura = (capture: AuraCapture) => {
    const newGallery = [capture, ...(preferences.auraGallery || [])];
    const progression = ProgressionService.addXP(preferences, 'ritual'); // Using ritual as proxy or generic? Ritual is fine.
    updatePreferences({ auraGallery: newGallery, xp: progression.xp, level: progression.level });
  };

  // Calculate Natal Chart once for all dashboard components
  useEffect(() => {
    // Only attempt calculation if context is fully hydrated from persistence
    if (!isHydrated) return;

    async function initChart() {
      if (preferences.birthDate && preferences.birthLocation) {
        console.log("Shell: Initializing Chart for", preferences.birthDate);
        try {
        ...
        console.log("Shell: Missing birth data - Calibration required.");
      }
    }
    initChart();
  }, [preferences.birthDate, preferences.birthLocation, isHydrated]);
      ...
      >
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
            ...
            <button 
                ...
                className="p-2 hover:bg-white/5 rounded-lg transition-colors text-white/50 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
            ...
            {navItems.map((item) => {
                ...
                return (
                    <button
                        ...
                        className={`w-full flex items-center justify-between p-4 rounded-xl transition-all group border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${
                            activeView === item.id 
            ...
            <div className="flex items-center gap-4">
                ...
                {(!user || user.isAnonymous) ? (
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-lg shadow-indigo-500/20 active:scale-95 ml-2"
                    >
                        Sign In
                    </button>
                ) : (
                    <div className="flex items-center gap-3 pl-2 border-l border-white/10 ml-2">
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] font-bold text-white uppercase tracking-wider">{user?.displayName || 'User'}</span>
                            <button 
                                onClick={logout}
                        ...
                        <div className="w-8 h-8 rounded-full bg-linear-to-br from-indigo-500 to-fuchsia-500 p-0.5">
                ...
                <button 
                    ...
                    className="p-2 hover:bg-white/10 rounded-full transition-colors text-fuchsia-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-500"
                    aria-label="Open Aura Camera"
                >
                    <Camera size={20} />
                </button>
                <button 
                    onClick={() => setIsCalibrationOpen(true)}
                    className="p-2 hover:bg-white/10 rounded-full transition-colors text-indigo-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                    aria-label="Open Cosmic Calibration Settings"
                >
                    <Settings size={20} />
                </button>
            </div>
        ...
        <main className="flex-1 overflow-auto p-6 relative scrollbar-hide">
            <AnimatePresence mode="wait">
                ...
                >
                    {activeView === 'compass' && (
                        <div className="h-full flex flex-col space-y-6 overflow-y-auto pr-2">
                             ...
                             <div className="space-y-6">
                                <div className="flex justify-between items-end">
                                    <div className="space-y-1">
                                        <h1 className="text-3xl font-bold text-white tracking-tight">
                                            {preferences.birthDate ? `Welcome back, ${preferences.name || "Traveler"}` : "Welcome, Traveler"}
                                        </h1>
                                    ...
                                    {preferences.birthDate ? (
                                        ...
                                        <button 
                                            onClick={() => setIsCalibrationOpen(true)}
                                            className="mb-1 text-[10px] uppercase tracking-widest text-white bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded-lg transition-all shadow-lg shadow-indigo-500/20 active:scale-95 animate-pulse"
                                        >
                                            Start Calibration
                                        </button>
                                    )}
                                ...
                                {/* Daily Pulse Card */}
                                {(() => {
                                    const pulse = preferences.birthDate 
                                        ? NumerologyEngine.getDailyPulse(preferences.birthDate)
                                        : { 
                                            message: "The universe is silent, awaiting your resonance. Calibrate your birth signature to reveal your daily guidance.", 
                                            context: "Once your coordinates are set, the Daily Pulse will calculate your personal vibration based on the current celestial alignments.",
                                            focus: "Discovery",
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                      {[
                                        { 
                                            label: "Life Path", 
                                            val: preferences.birthDate ? NumerologyEngine.calculateLifePath(preferences.birthDate).core : null, 
                                            color: "text-amber-400",
                                            borderColor: "border-amber-500/30",
                                            bg: "bg-amber-950/10",
                                            desc: "The road you're traveling. Your core purpose.",
                                            source: preferences.birthDate ? `Birth Date: ${new Date(preferences.birthDate).toLocaleDateString()}` : "Calibration Required"
                                        },
                                        { 
                                            label: "Destiny", 
                                            val: (preferences.fullName || preferences.name) ? NumerologyEngine.calculateName(preferences.fullName || preferences.name || "", 'pythagorean').core : null, 
                                            color: "text-cyan-400",
                                            borderColor: "border-cyan-500/30",
                                            bg: "bg-cyan-950/10",
                                            desc: "Your potential. What you're destined to become.",
                                            source: (preferences.fullName || preferences.name) ? `Birth Name: ${preferences.fullName || preferences.name}` : "Calibration Required"
                                        },
                                        { 
                                            label: "Active", 
                                            val: preferences.name ? NumerologyEngine.calculateName(preferences.name, 'chaldean').core : null, 
                                            color: "text-pink-400",
                                            borderColor: "border-pink-500/30",
                                            bg: "bg-pink-950/10",
                                            desc: "Your daily energy. How you interact right now.",
                                            source: preferences.name ? `Chosen Name: ${preferences.name}` : "Calibration Required"
                                        }
                                      ].map((num, i) => {
                                         const valDisplay = num.val !== null ? num.val : "??";
                                         const details = num.val !== null ? NumerologyEngine.getRichDetails(num.val) : { title: "Unrevealed", description: "" };
                                         ...
                                         <button 
                                            key={i} 
                                            onClick={() => num.val !== null && setSelectedNum({number: num.val as number, type: num.label as 'Life Path' | 'Destiny' | 'Active' | 'Personal Day', source: num.source})}
                                            disabled={num.val === null}
                                            className={`relative overflow-hidden rounded-2xl border ${num.borderColor} ${num.bg} p-6 flex flex-col items-start justify-between min-h-[160px] ${num.val !== null ? 'hover:scale-[1.02]' : 'cursor-not-allowed opacity-80'} transition-all group text-left w-full hover:shadow-2xl hover:shadow-${num.color.replace('text-', '')}/20`}
                                         >
                                             {/* Header */}
                                             ...
                                             <div className="z-10 mt-2 opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-4 right-4 bg-black/90 px-3 py-1.5 rounded-full text-[9px] text-emerald-400 border border-emerald-500/30 font-bold tracking-widest uppercase shadow-xl">
                                                {num.val !== null ? "Tap to Decode" : "Requires Calibration"}
                                             </div>
                                         </button>
                                     )})}
                                </div>
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[500px]">
                                 <div className="lg:col-span-2 flex flex-col items-center justify-center relative bg-black/40 rounded-2xl border border-white/5 overflow-hidden">
                                      {natalChart ? (
                                        <NatalCompass chart={natalChart} />
                                      ) : (
                                        <div className="flex flex-col items-center justify-center p-12 space-y-6 text-center">
                                            <div className="relative">
                                                <Compass className="text-indigo-400/50 animate-spin-slow" size={120} />
                                                <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-300 animate-pulse" size={48} />
                                            </div>
                                            <div className="space-y-2">
                                                <h3 className="text-xl font-bold text-white tracking-widest uppercase">Calibrate Your Soul</h3>
                                                <p className="text-slate-400 text-sm max-w-md mx-auto leading-relaxed">
                                                    Your astrological coordinates are currently ungrounded. Enter your birth details to unlock your personal Natal Compass and start your journey.
                                                </p>
                                            </div>
                                            <button 
                                                onClick={() => setIsCalibrationOpen(true)}
                                                className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold uppercase tracking-[0.2em] text-xs transition-all shadow-xl shadow-indigo-500/20 active:scale-95 group flex items-center gap-3"
                                            >
                                                Start Calibration
                    ...
                    {activeView === 'codex' && <GrimoireCodex />}
                ...
                >
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            ...
                            {/* Unlock Hint */}
                            <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-6 space-y-2">
                                <h4 className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                                    <Sparkles size={12} className="text-amber-400" /> Current Quest
                                </h4>
                                <p className="text-sm text-indigo-100/80 leading-relaxed italic">
                                    &ldquo;{ProgressionService.getUnlockHint(preferences.level)}&rdquo;
                                </p>
                            </div>
                        </div>
                        <button 
