...
const GrimoireCodex: React.FC<GrimoireCodexProps> = ({ isOpen = true, onClose, mode = 'modal' }) => {
    ...
    // For modal mode, check if open
    if (mode === 'modal' && !isOpen) return null;

    const content = (
        <motion.div 
            initial={{ opacity: 0, scale: mode === 'modal' ? 0.9 : 1 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: mode === 'modal' ? 0.9 : 1 }}
            className={`${mode === 'modal' ? 'w-full max-w-5xl h-[80vh]' : 'w-full h-full'} bg-slate-950 ${mode === 'modal' ? 'border border-emerald-900/50 rounded-3xl' : ''} overflow-hidden flex flex-col shadow-2xl relative`}
        >
            {/* Header */}
    ...
    // Embedded mode returns content directly
    return content;
};
